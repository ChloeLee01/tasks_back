...
exports.updateTask = async (request, response) => {
  const { title, description, date, isCompleted, isImportant, id } =
    request.body;
  console.log(title, description, date, isCompleted, isImportant, id);
  // body에 요청한 데이터 확인용 코드
  ...
  try {
    ...
    return response.status(201).json({ msg: "데이터가 수정되었습니다." });
  } catch (error) {
...
exports.updateCompletedTask = async (request, response) => {
  ...
  try {
    ...
    return response.status(201).json({ msg: "완료여부가 변경되었습니다." });
  } catch (error) {
    return response.status(500).json({ msg: "데이터 입력 에러: ", error });
  }
};
